...
import ViewRequest from '../components/ViewRequest'; // ✅ NEW IMPORT
...
const Dashboard = () => {
  ...
  const [viewingRequest, setViewingRequest] = useState(null); // ✅ NEW STATE
  ...
  // ✅ NEW: from view modal, switch to edit modal
  ...
  const readyCount = requests.filter(r => ['Approved', 'Ready', 'Ready for pickup'].includes(r.status)).length;
      ...
      <div className="flex-1 ml-72">
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 px-10 py-5 flex justify-between items-center sticky top-0 z-10">
          <div className="relative w-96">
            ...
            <input type="text" placeholder="Search document type or ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
...
// ✅ UPDATED: row is now fully clickable for viewing; edit button still works independently
const RequestItem = ({ req, onView, onEdit }) => {
  ...
  const getRemarksString = () => {
    if (!remarks) return "No specific feedback provided.";
    return typeof remarks === 'object' ? JSON.stringify(remarks) : remarks;
  };

  return (
    // ✅ Entire row is clickable to open view modal
    <div
      onClick={onView}
      className="group flex flex-col p-5 rounded-2xl border border-slate-100 hover:border-blue-200 hover:bg-blue-50/30 transition-all bg-white cursor-pointer"
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {status === 'Pending' && (
            // ✅ stopPropagation so edit button doesn't also trigger view
            <button
